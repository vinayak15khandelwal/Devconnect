...
describe("SkillsGrid", () => {
  it("shows an empty state when there are no skills", () => {
    renderWithProviders(<SkillsGrid skills={[]} username="alice" canEndorse={false} />);
  ...
  it("does not show endorse controls when canEndorse is false", () => {
    renderWithProviders(
      <SkillsGrid
        skills={[{ id: "1", name: "React", endorsementCount: 0, endorsedByMe: false }]}
        username="alice"
        canEndorse={false}
      />
    );
  ...
  it("shows a checkmark instead of an endorse button once already endorsed", () => {
    renderWithProviders(
      <SkillsGrid
        skills={[{ id: "1", name: "React", endorsementCount: 1, endorsedByMe: true }]}
        username="alice"
        canEndorse={true}
      />
    );
