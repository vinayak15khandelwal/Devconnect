...
export default function BlogEditor() {
  ...
  const { data: existing } = useQuery({
    ...
    queryFn: async () => (await api.get<{ data: BlogPost }>(`/api/blog/${slug}`)).data.data,
    enabled: isEditing,
  });
  ...
  const savePost = useMutation({
    ...
    onError: (err: any) => setError(err?.response?.data?.message || "Couldn't save the post."),
  });
    ...
    <div className="min-h-screen bg-gray-50 px-6 py-10">
      <div className="max-w-2xl mx-auto bg-white rounded-xl border border-gray-100 shadow-sm p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit" disabled={savePost.isPending}
