...
export default function BlogPost() {
  ...
  if (error || !post)
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        <Navbar />
        <div className="flex items-center justify-center text-gray-400 dark:text-gray-500 py-20">Post not found.</div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        <article className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-8">
          <Link to="/blog" className="text-sm text-brand-600 dark:text-brand-400 hover:underline">
            ← All posts
          </Link>
          ...
          <div className="flex items-start justify-between mt-4">
            <div>
              ...
              <div className="flex items-center gap-2 mt-2 text-sm text-gray-400 dark:text-gray-500">
                <Link to={`/u/${post.author?.username}`} className="hover:text-brand-600 dark:hover:text-brand-300">
                  {post.author?.name}
                </Link>
                <span>·</span>
            ...
            {isAuthor && (
              <div className="flex gap-3 text-sm shrink-0">
                <Link to={`/blog/${post.slug}/edit`} className="text-brand-600 dark:text-brand-400 hover:underline">
                  Edit
                </Link>
                <button
